...
class DAgger():
    def __init__(self, teacher, student, env, demostrations, initial_beta:int=150,demos_per_beta=500, max_depth=8):
    ...
    def train(self, teacher, env):
        ...
        for count in range(Math.floor(4/3*self.initial_beta)):
            ...
            np.random.shuffle(trajectories['next_obs'])
            
            if isinstance(teacher, BaseAlgorithm):
                acts, _ = teacher.predict(trajectories['next_obs'])
            elif isinstance(teacher, Callable):
                acts, _ = teacher(trajectories['next_obs']), None
            else:
                raise Exception("expert policy must be instance of BaseAlgorithm or a Callable") 
            
            self.demos['obs'].extend(trajectories['next_obs'])
            self.demos['acts'].extend(acts)
            self.student = tree.DecisionTreeClassifier(max_depth=self.max_depth)
