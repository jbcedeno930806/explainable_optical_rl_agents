...
from explainable.utils import Service, Path, get_k_shortest_paths, get_path_weight
...
class OpticalNetworkEnv(gym.Env):
    ...
    def __init__(self, topology=None, episode_length=1000, load=10, mean_service_holding_time=10800.0,
        ...
        if topology is None:
            ...
            assert k_paths <= self.topology.graph['k_paths']
            # self.k_paths = self.topology.graph['k_paths']
            # self.k_shortest_paths = self.topology.graph['ksp']
            self.k_paths = k_paths
            ksp = self.topology.graph['ksp']
            self.k_shortest_paths = {key:ksp[key][:k_paths] for key in ksp.keys()}
